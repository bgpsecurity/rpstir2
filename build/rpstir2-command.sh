...
function helpFunc()
{
    ...
    echo -e "4) ./rpstir2-command.sh states:               when result shows 'state:end', it means rsync/rrdp is end" 
    echo -e "5) ./rpstir2-command.sh reset:                it will reset all data in mysql and in local cache" 
    echo -e "6) ./rpstir2-command.sh parsefile xxx.cer:    it will parse and validate the file(*.cer/*.mft/*.roa/*.crl)"

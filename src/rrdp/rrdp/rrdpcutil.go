...
func rrdpByUrlImpl(notifyUrl string, destPath string, syncLogId uint64,
	...
	belogs.Debug("rrdpByUrlImpl():notifyUrl canDelta:", notifyUrl, canDelta)
	...
	if !canDelta {
		snapshotDeltaResult = rrdpmodel.SnapshotDeltaResult{
			...
			DestPath:   destPath,
			LastSerial: 0}
		belogs.Debug("rrdpByUrlImpl(): will snapshot:", notifyUrl, snapshotDeltaResult)
		...
		belogs.Debug("rrdpByUrlImpl(): will delta:", notifyUrl, snapshotDeltaResult)
...
func rrdpByUrl(rrdpModelChan rrdpmodel.RrdpModelChan) {
	...
	if !needRrdp {
		belogs.Debug("RrdpByUrl():no need rrdp:", rrdpModelChan.Url)
		return
	}
...
func parseCerAndGetRpkiNotify(cerFile string) (RpkiNotify string) {
	...
	if err != nil {
		...
		belogs.Error("parseCerAndGetRpkiNotify(): filerepo file connecteds failed:", cerFile, "   err:", err)
		return ""
	}

...
func validateCrl(chains *Chains, crlId uint64, wg *sync.WaitGroup, chainCrlCh chan int) {
	...
	if err != nil {
		...
		if len(sameAkiChainCrls) == 1 {
			...
			smallerFiles := make([]ChainCrl, 0)
			biggerFiles := make([]ChainCrl, 0)
...
func getCrlParentChainCer(chains *Chains, crlId uint64) (chainCerAlone ChainCerAlone, err error) {
	chainCrl, err := chains.GetCrlById(crlId)
	if err != nil {
...
func getSameAkiChainCrls(chains *Chains, crlId uint64) (sameAkiChainCrls []ChainCrl, err error) {
	chainCrl, err := chains.GetCrlById(crlId)
	if err != nil {
	...
	sameAkiChainCrls = make([]ChainCrl, 0)

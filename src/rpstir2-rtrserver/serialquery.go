...
func ProcessSerialQuery(rtrPduModel RtrPduModel) (serialResponses []RtrPduModel, err error) {
	...
	if len(serialNumbers) == 0 {
		...
		rtrIncrementals, sessionId, serialNumber, err := GetRtrIncrementalAndSessionIdAndSerialNumber(clientSerialNumber)
		if err != nil {
...
func needResetQuery(clientSessionId uint16, clientSerialNumber uint32) (serialNumbers []uint32, err error) {
	...
	serialNumbers, err = GetSpanSerialNumbers(clientSerialNumber)
...
func assembleSerialResponses(rtrIncrementals []model.LabRpkiRtrIncremental, protocolVersion uint8, sessionId uint16,
	serialNumber uint32) (rtrPduModels []RtrPduModel, err error) {
...
func convertRtrIncrementalToRtrPduModel(rtrIncremental *model.LabRpkiRtrIncremental,
	...
	if ipType == iputil.Ipv4Type {
		ipv4 := [4]byte{0x00}
		copy(ipv4[:], ipHex[:])
		rtrIpv4PrefixModel := NewRtrIpv4PrefixModel(protocolVersion, GetIpPrefixModelFlags(rtrIncremental.Style),
		...
		ipv6 := [16]byte{0x00}
		copy(ipv6[:], ipHex[:])
		rtrIpv6PrefixModel := NewRtrIpv6PrefixModel(protocolVersion, GetIpPrefixModelFlags(rtrIncremental.Style),

...
func extractRoaOidImpl(secPacketSeq *Packet, certFile string, roaModel *model.RoaModel) (err error) {
	...
	asnInt64, ok := asId.Value.(int64)
	if ok {
		roaModel.Asn = asnInt64
	} else {
		asnUint64, ok := asId.Value.(uint64)
		if ok {
			roaModel.Asn = int64(asnUint64)
		} else {
			belogs.Error("extractRoaOidImpl(): asId.Value is not int64/uint64: ", asId.Value)
			return errors.New("asId.Value is not int64/uint64")
		}
	}
...
func reExtractRoaOid(fileByte []byte, certFile string, roaModel *model.RoaModel) (err error) {
	...
	*/
	...
	datalen, datapos, _ = asn1util.DecodeFiniteAndInfiniteLen(enum0)
	belogs.Debug("reExtractRoaOid():enum0 pos:", datapos, "  datalen:", datalen)

	//avoid error of 0x00, 0x00, so it is not limit datalen, and will include all data
	oct0 := enum0[datapos:]
	//logs.LogDebugBytes(("reExtractRoaOid():oct0:", oct0)
	datalen, datapos, _ = asn1util.DecodeFiniteAndInfiniteLen(oct0)
	belogs.Debug("reExtractRoaOid():oct0 pos:", datapos, "  datalen:", datalen)

	//avoid error of 0x00, 0x00, so it is not limit datalen, and will include all data
	oct1 := oct0[datapos:]
	//logs.LogDebugBytes(("reExtractRoaOid():oct1:", oct1)
	datalen, datapos, _ = asn1util.DecodeFiniteAndInfiniteLen(oct1)
	...
	if seq0Class != byte(0x30) {
		...
		datalen, datapos, _ = asn1util.DecodeFiniteAndInfiniteLen(seq0)

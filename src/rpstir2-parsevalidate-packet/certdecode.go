...
func DecodePacket(data []byte) *Packet {
	topHierarchyFor00 := asn1util.GetTopHierarchyFor00(data)
...
func decodePacketImpl(data []byte, hierarchyFor00 int, topHierarchyFor00 int) (*Packet, []byte, error) {
	...
	data = asn1util.TrimPrefix00(data)
	if len(data) == 0 {
		return nil, nil, nil
	}
	var err error
	...
	datalen := asn1util.DecodeInteger(data[1:2])
	...
	if datalen == 0x80 {
		...
		lastpos, err := asn1util.IndexEndOfBytes(data, p.TagType, hierarchyFor00, topHierarchyFor00)
		...
		/* */
		datalen, datapos, err = asn1util.DecodeFiniteLen(data)
		if err != nil {
			return nil, nil, nil
		}
	...
	valueDataTmp := asn1util.TrimPrefix00(valueData)
	...
	*/
	...
	if p.TagType == TypeConstructed {
		...
		case TagEOC:
		case TagBoolean:
			val := asn1util.DecodeInteger(valueData)
			p.Value = val != 0
		case TagInteger:
			p.Value = asn1util.DecodeInteger(valueData)
		case TagBitString:
			p.Value = (valueData)
		case TagOctetString:
		...
		case TagObjectIdentifier:
			p.Value = asn1util.DecodeOid(valueData)
		...
		case TagEmbeddedPDV:
		case TagUTF8String:
			p.Value = asn1util.DecodeUTF8String(valueData)
		case TagRelativeOID:
		case TagSequence:
		case TagSet:
		case TagNumericString:
		case TagPrintableString:
			p.Value = asn1util.DecodeString(valueData)
		case TagT61String:
		case TagVideotexString:
		case TagIA5String:
			p.Value = asn1util.DecodeIA5String(valueData)
		case TagUTCTime:
			p.Value, err = asn1util.DecodeUTCTime(valueData)
		case TagGeneralizedTime:
			p.Value, err = asn1util.DecodeGeneralizedTime(valueData)
...
func (p *Packet) Bytes() []byte {
	...
	packetLength := asn1util.EncodeInteger(p.DataLength())

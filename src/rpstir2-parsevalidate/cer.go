...
func parseCerModel(certFile string, cerModel *model.CerModel, stateModel *model.StateModel) (err error) {
	...
	if err != nil {
		belogs.Error("parseCerModel():ReadFileAndDecodeBase64 err: ", certFile, err)
...
func validateCerlModel(cerModel *model.CerModel, stateModel *model.StateModel) (err error) {
	...
	*/
	...
	if len(split) > 2 {
		...
		// must contain CN, may contain serialnumber, no other
		foundCN := false
		foundOther := false
	...
	// rescert_aki_chk myssl.c P2247  TODO ? aki.issuer, aki.serial ?
	if !cerModel.IsRoot {
...
func ParseCerSimpleModel(certFile string) (parseCerSimple model.ParseCerSimple, err error) {
	// results will be used
	belogs.Debug("ParseCerSimpleModel(): certFile:", certFile)
	results, err := opensslutil.GetResultsByOpensslX509(certFile)
	if err != nil {
		belogs.Error("ParseCerSimpleModel(): GetResultsByOpensslX509: err: ", err, ": "+certFile)
		return parseCerSimple, err
	}
	belogs.Debug("ParseCerSimpleModel(): GetResultsByOpensslX509 len(results):", certFile, len(results))

	//  SIA
	_, siaModel, err := openssl.ParseAiaModelSiaModelByOpensslResults(results)
	if err != nil {
		belogs.Error("ParseCerSimpleModel(): ParseAiaModelSiaModelByOpensslResults: certFile,  err: ", certFile, err)
		return parseCerSimple, err
	}
	belogs.Debug("ParseCerSimpleModel():certFile, siaModel ", certFile, siaModel)
	...
	if err != nil {
		belogs.Error("ParseCerSimpleModel(): ReadFileAndDecodeBase64:certFile,  err: ", certFile, err)
		return parseCerSimple, err
	}
	cer, err := x509.ParseCertificate(fileDecodeBase64Byte)
	if err != nil {
		belogs.Error("ParseCerSimpleModel(): ParseCertificate: certFile,  err: ", certFile, err)
		return parseCerSimple, err
	}
